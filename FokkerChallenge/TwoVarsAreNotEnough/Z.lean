...
theorem Z_lc_closed {M}
  (hm : ClosedUnderApp fvar_or_combinator M):
  ∀ t, ClosedUnderApp (Z M) t ->  t.LC := by
  intros x hx
  induction hx with grind [Z_lc]
...
theorem no_x_exists_z {M N}
  ...
  (steps : unroll M N)
  (hx : "x" ∉ N.fv) :
  ∀ N, ClosedUnderApp (Z M) N -> ∃ N', N ↠βᶠ N' /\ "x" ∉ N'.fv := by
  intros N hN
  induction hN with
  | base hN =>  rcases hN with _|_|hN|_
                . subst_vars
                  refine ⟨fvar "y", by grind⟩
                . subst_vars
                  refine ⟨fvar "z", by grind⟩
  ...
  | app h5 h6 iha ihb =>  obtain ⟨c, h1, h2⟩ := iha
                          ...
                          . apply Z_lc_closed hm _ h6
                          . have := Z_lc_closed hm _ h5
                            cases FullBeta.steps_lc_or_rfl h1 with grind

theorem step_closedUnderApp_unroll_z {M N}
  (hm : ClosedUnderApp fvar_or_combinator M)
  ...
  contain_x N' /\ ClosedUnderApp (Z (M.app (fvar "x"))) N' /\ ClosedUnderApp fvar_or_combinator N' := by
    ...
    . intros _ steps
      apply h_contain_x _ (.trans (FullBetaEta.from_beta _ _ (HeadReduction2.step_2_beta step (Z_lc_closed (by grind) _ ih))) steps)
    . obtain ⟨l, f, h3, h4, h5⟩ := closedunderapp_multiapp ih
      rcases h4 with h4|h4|h4|⟨h4, h6⟩
      . subst_vars
        exfalso
